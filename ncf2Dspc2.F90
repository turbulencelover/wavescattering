...
module ncf2Dspc2
  ...
  integer, parameter :: putE = 0, putKE = 0,putPE = 0, putGE = 1, putAE = 1, putBF = 0
  ...
  integer, parameter :: numkh =floor(2*ktx/3.0)-1, numkz =2*(floor(2*ktz/3.0)-1)
  !! >>NOTE<< in this module positive and negative kz's are stored separate! 
  
  !! the frequency of outputting
  integer, parameter :: nsp2dout = 30          ! number of time 2D spectrum is dumped in output file
...
subroutine prep_ncf2Dspc()
  ...
  call check_ncf2( nf90_def_dim(idspc2d,"kz",numkz+1,idkz2) )
...
subroutine dump_ncf2Dspc(zx,zy,zz,tt,ux,uy,uz,ge,g1,g2,zxwv,zywv,zzwv,ttwv)
  ...
  real, dimension(1:numkh,1:numkz+1) :: Ejunk
  complex, dimension(:,:,:), allocatable :: gewv,g1ge,g2ge
  real, dimension(:,:,:), allocatable    :: spc2d,spctot2d
  integer, dimension(1:numkh,1:numkz+1)    :: nm2d,nmtot2d
  real, dimension(1:numkh,1:numkz+1)       :: khkh,kzkz 
  ...
  if (putKEWV+putPEWV+putKEMF+putPEMF.ne.0) then
     ...
     allocate(g1ge(iktx,ikty,iktzp))
     allocate(g2ge(iktx,ikty,iktzp))
     allocate(spc2d(1:numkh,1:numkz+1,10),spctot2d(1:numkh,1:numkz+1,10))  
  else
     allocate(spc2d(1:numkh,1:numkz+1,6),spctot2d(1:numkh,1:numkz+1,6))
  endif
  
  ncoll  = (numkh)*(numkz+1)
  ncoll6 = (numkh)*(numkz+1)*6
  ncoll10 = (numkh)*(numkz+1)*10
  nm2d    = 0
  spc2d   = 0.0
  Ejunk   = 0
  ...
  do ikz=1,iktzp
     ...
     do iky=1,ikty
        ...
        do ikx=1,iktx
           ...
           jh  = nint(wkh*L1/twopi) + 1
           jv  = nint(kzn)+numkz/2.0 + 1          
           jv2 = nint(-kzn)+numkz/2.0 + 1
           
           if (((jv.le.(numkz+1)).and.(jh.le.numkh)).and.(L(ikx,iky,ikz).eq.1)) then           
              wk   = max(wk,  1.e-10)
              wkh2 = max(wkh2,1.e-10)
...
! k \in R_k
              if (wkh2n.gt.1.e-10 .and. abs(kzn).gt.1.e-10) then
                 ...
                 spc2d(jh,jv2,5) = spc2d(jh,jv2,5) + vz/wkh2
              endif
...
! Buoyancy Flux
  ...
  if (putKEWV+putPEWV+putKEMF+putPEMF.ne.0) then 
     gewv=cmplx(0.,0.)
     g2ge=cmplx(0.,0.)
     call atowb(gewv,g1,g2ge,zxwv,zywv,zzwv,ttwv,ux,uy,uz)
     do ikz=1,iktzp
        ikza = mype*iktzp+ikz
        kz = kza(ikza)
        kzn = kz*L3/twopi
     ...
     gewv=cmplx(0.,0.)
     g1ge=cmplx(0.,0.)
     call atowb(gewv,g1ge,g2,zxwv,zywv,zzwv,ttwv,ux,uy,uz)
     do ikz=1,iktzp
        ikza = mype*iktzp+ikz
        kz = kza(ikza)
        kzn = kz*L3/twopi
        do iky=1,ikty
           ...
           do ikx=1,iktx
              ...
              if (((jv.le.(numkz+1)).and.(jh.le.numkh)).and.(L(ikx,iky,ikz).eq.1)) then           
                 ...
                 vzz      = real( zzwv(ikx,iky,ikz)*conjg(zzwv(ikx,iky,ikz)) )
                 vt       = real( ttwv(ikx,iky,ikz)*conjg(ttwv(ikx,iky,ikz)) )
                 spc2d(jh,jv2,7) = spc2d(jh,jv2,7) + vzx/wk**2 + vzy/wk**2 + vzz/wk**2
                 spc2d(jh,jv2,8) = spc2d(jh,jv2,8) + vt*aj/bj
              endif
           enddo
        enddo
     enddo
     g1ge=cmplx(0.,0.)
     g2ge=cmplx(0.,0.)
     call atowb(ge,g1ge,g2ge,zxwv,zywv,zzwv,ttwv,ux,uy,uz)
     do ikz=1,iktzp
        ikza = mype*iktzp+ikz
        kz = kza(ikza)
        kzn = kz*L3/twopi
        do iky=1,ikty
           ...
           do ikx=1,iktx
              ...
              if (((jv.le.(numkz+1)).and.(jh.le.numkh)).and.(L(ikx,iky,ikz).eq.1)) then           
                 ...
                 vzz      = real( zzwv(ikx,iky,ikz)*conjg(zzwv(ikx,iky,ikz)) )
                 vt       = real( ttwv(ikx,iky,ikz)*conjg(ttwv(ikx,iky,ikz)) )
                 spc2d(jh,jv,9) = spc2d(jh,jv,9) + vzx/wk**2 + vzy/wk**2 + vzz/wk**2
                 spc2d(jh,jv,10) = spc2d(jh,jv,10) + vt*aj/bj
              endif
           enddo
        enddo
     enddo
