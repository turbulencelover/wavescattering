...
! -----------------------------------------------------------------
  ...
  integer, parameter :: myrest =   1                 ! used in random seed
...
! -----------------------------------------------------------------
  ...
  external :: init_params,convol,constr,ranno,forset,force
...
! -----------------------------------------------------------------
  ...
  call init_params()
  call init_cond(zxok,zyok,zzok,ttok,uk,vk,wk,geok,gw1k,gw2k,zxor,zyor,zzor,ur,vr,wr,ttor,ts)
...
! -----------------------------------------------------------------
  if (mype.eq.0) then
    ...
    print*,'       Output frequency = ', ndiagevery*delt, ' = ', ndiagevery,' dt.'
    print*,'  '
!!$    if (irest.eq.0) then
...
! -----------------------------------------------------------------
  ...
  call dump_wv3Dspc(gw1k,gw2k)
  
  if (ispc2d < nsp2dout) then
     call dump_ncf2Dspc(zxnk,zynk,zznk,ttnk,uk,vk,wk,geok,gw1k,gw2k,rhzx,rhzy,rhzz,rhtt)
  endif
...
! -----------------------------------------------------------------
    ...
    if ((nt==ncwrite).and.(nt.ne.0)) then
    ...
    if ( mod(nt,nwsevery).eq. 0) then
       call dump_wv3Dspc(gw1k,gw2k)
    endif
    
    
    if  ( mod(nt,nsp2devery).eq. 0 ) then
       call dump_ncf2Dspc(zxnk,zynk,zznk,ttnk,uk,vk,wk,geok,gw1k,gw2k,rhzx,rhzy,rhzz,rhtt)
    endif
...
subroutine out2(zx,zy,zz,tt,ux,uy,uz,ge,g1,g2,nt)
