...
! -----------------------------------------------------------------  
  ...
  real,    parameter :: ki     =   (L1/twopi) * (36.)    ! wavenumber for initial conditions 
...
! Open files for diagnostics
  if (mype.eq.0) then
    ...
    if ( dumpslices.eq.1) then
       open (301,file='bbHC.out', form='formatted')
       open (302,file='bbVC.out', form='formatted')
       open (303,file='zzHC.out', form='formatted')
       open (304,file='zzVC.out', form='formatted')
       open (305,file='zyHC.out', form='formatted')
       open (306,file='zyVC.out', form='formatted')
    end if
  endif
...
! -----------------------------------------------------------------
  ...
  call killwaves(zxok,zyok,zzok,ttok,geok,gw1k,gw2k,uk,vk,wk)
...
!    call cfl(zxok,zyok,zzok,uk,vk,wk,ur,vr,wr)
    ...
    zznk = zzok
    ttnk = ttok
    call  pvcomp(zxnk,zynk,zznk,ttnk,uk,vk,wk,zxnr,zynr,zznr,ttnr,ur,vr,wr)
    if ( dumpslices.eq.1) then
       call  dumpHCVC(ttnk,ttnr,301,302,4,1,1)
       call  dumpHCVC(zznk,zznr,303,304,3,1,1)
       call  dumpHCVC(zynk,zynr,305,306,2,1,1)
    endif
  endif ! irest
